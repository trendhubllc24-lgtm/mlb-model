...
function findBestSplit(X, residuals, weights, idxAll) {
  ...
  const parentSse = sseFromSums(totalSum, totalSumSq, n);
  ...
  // gain = how much this split actually reduced SSE vs. leaving the node
  // unsplit — the raw ingredient feature-importance sums across every tree.
  if (best) best.gain = Math.max(0, parentSse - best.sse);
  return best;
}

function buildTree(X, residuals, weights, idxAll, depth, importance) {
  ...
  if (importance) importance[best.f] = (importance[best.f] || 0) + best.gain;
  return {
    f: best.f, threshold: best.threshold,
    left: buildTree(X, residuals, weights, best.leftIdx, depth + 1, importance),
    right: buildTree(X, residuals, weights, best.rightIdx, depth + 1, importance),
  };
}
...
export function trainGBoost(X, y) {
  ...
  const importance = Array(nFeatures).fill(0); // total SSE-reduction credited to each feature index
  ...
  for (let round = 0; round < N_ROUNDS; round++) {
    ...
    const tree = buildTree(X, residuals, weights, allIdx, 0, importance);
    trees.push(tree);
  ...
  // Normalize to a 0-1 share of total gain so it reads the same regardless
  // of dataset size or round count — "this feature accounted for 18% of
  // every SSE-reducing split across the whole forest."
  const totalGain = importance.reduce((a, b) => a + b, 0) || 1;
  const featureImportance = importance.map((g) => g / totalGain);

  return { model: { F0, trees, learningRate: LEARNING_RATE }, trainAccuracy: correct / n, featureImportance };
}
