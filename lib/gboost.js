...
// Bumped from depth 3 / 60 rounds now that the model has 10 features
// instead of 4 (added 2026-07-02) — the old settings were sized for a much
// smaller feature space and couldn't find weaker interactions (e.g.
// bullpen ERA only mattering in combination with rest days) even if they
// exist. Learning rate lowered to compensate for the extra rounds, so the
// added capacity goes toward finding real signal instead of overfitting
// faster. MIN_LEAF raised slightly too, since deeper trees + same ~14.6k
// rows means smaller leaves are more likely to just be memorizing noise.
const MAX_DEPTH = 4;
const N_ROUNDS = 120;
