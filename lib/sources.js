...
import { buildPitcherLogs, pitcherEraAsOf } from "./pitcherForm.js";
...
export async function getGamePitchers(eventId) {
  try {
    ...
    for (const p of box.players || []) {
      ...
      const ipRaw = ipIdx >= 0 ? starter.stats?.[ipIdx] : null;
      const er = erIdx >= 0 ? Number(starter.stats?.[erIdx]) : null;

      let ip = null;
      if (ipRaw != null) {
        // MLB box-score IP notation: "5.1" = 5 innings + 1 out (not .1 decimal)
        const [whole, part] = String(ipRaw).split(".").map(Number);
        ip = (whole || 0) + (part || 0) / 3;
      }
      if (ip != null && er != null) {
        result[homeAway] = { name: starter.athlete.displayName, ip, er };
      }
    }
...
export async function updateTrackRecord(redis, slate, homeAdvMap) {
  ...
  const gLogs = gModel ? buildTeamGameLogs(predictions) : null;
  const pitcherStarts = gModel ? (await redis.get("mlb-pitcher-starts")) || {} : {};
  const pitcherLogs = gModel ? buildPitcherLogs(pitcherStarts) : null;
  ...
  for (const g of slate) {
    ...
    if (!predictions[gid]) {
      ...
      if (gModel) {
        const formDiff = recentForm(gLogs[g.home], g.date) - recentForm(gLogs[g.away], g.date);
        // g.probableHome/probableAway come straight from ESPN's pre-game
        // "probable starter" field — pitcher name only, no ID, which is
        // exactly what pitcherLogs is keyed by.
        const pitcherDiff = pitcherLogs
          ? pitcherEraAsOf(pitcherLogs, g.probableAway, g.date) - pitcherEraAsOf(pitcherLogs, g.probableHome, g.date)
          : 0;
        const gProb = predictGBoost(gModel, [rH - rA, homeAdvUsed, formDiff, pitcherDiff]);
